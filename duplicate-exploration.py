...
if not os.path.exists('lib/duplicate_galaxies.npy'):
    groups = find_duplicates()
    np.save('lib/duplicate_galaxies.npy', groups)
else:
    groups = np.load('lib/duplicate_galaxies.npy')

axratios = np.zeros((len(groups), 2))
pitch_angles = np.zeros((len(groups), 2))

for dr7objid, id0, id1 in groups:
    with gu.get_model
    model0 = json.
