...
for chosen_id in id_list:
    ...
    if not os.path.isdir('pipeline_plots/{}'.format(chosen_id)):
        os.mkdir('pipeline_plots/{}'.format(chosen_id))
    ...
    out = plotting.make_pipeline_plots(
        ...
        distances=distances,
        # clean_points=True,
        file_loc='pipeline_plots/{}'.format(chosen_id)
    )

    plotting.combine_plots(
        'pipeline_plots/{}'.format(chosen_id),
        title='Subject ID: {}'.format(chosen_id)
    )
    try:
        shutil.copy2(
            'pipeline_plots/{}/combined.png'.format(chosen_id),
            'model-comparison-plots/{}.png'.format(chosen_id)
        )
